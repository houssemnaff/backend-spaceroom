...
const fileFilter = (req, file, cb) => {
    const allowedTypes = ['video/mp4', 'application/pdf', 'image/jpeg', 'image/png', 'application/octet-stream'];
...
const uploadfile = multer({ 
    ...
    fileFilter,
    limits: { 
      fileSize: 50 * 1024 * 1024 // Limite la taille des fichiers à 50 Mo
    }
  ...
  const uploadMiddleware = (req, res, next) => {
    uploadfile(req, res, (err) => {
      if (err instanceof multer.MulterError) {
        // Erreur de Multer (taille de fichier, etc.)
